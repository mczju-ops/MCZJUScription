...
public final class InscriptionShopMenu extends Menu {
  ...
  protected void setup() {
    ...
    boolean permanentUsed = state.hasBoughtPermanentThisTurn();
    ...
    setSlot(
        ...
        ItemBuilder.of(Material.GOLD_INGOT)
            ...
            .lore(
                List.of(
                    "<gray>骨币: <gold>" + bones + "  <gray>腐肉: <red>" + blood,
                    "<aqua>常驻×4: <white>每回合总共只能买 1 张"
                        + (permanentUsed ? " <red>(已购)" : ""),
    ...
    for (int i = 0; i < ShopLayout.PERMANENT.length; i++) {
      ...
      placeOffer(slot, offer, bones, ShopPurchaseLane.PERMANENT, permanentUsed, -1);
    }
    ...
    setSlot(
        ...
        ItemBuilder.of(Material.BARRIER)
            .customName("<red>关闭")
            .lore(List.of("<gray>可继续购卡", "<gray>潜行+右键商店工具进入出牌"))
            .build(),
  ...
  private void placeOffer(
    ...
    setSlot(
        ...
        (p, e) -> {
          if (blocked) {
            match.feedback().actionBarWarn("<yellow>本回合已从常驻货架购卡。");
            return;
          }
          if (!affordable) {
